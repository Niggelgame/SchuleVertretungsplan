...
class ApiProvider {
  ...
  Future<String> getData(String apiKey) async {
    final response = await client
      .get("http://http://192.168.19.66:5000/api/data",//"http://10.0.2.2:5000/api/data",
      headers: {
  ...
  Future<bool> signinUser(String username, String password, String apiKey) async {
    ...
    try{
      final response = await client
        .post("http://http://192.168.19.66:5000/api/signin",//"http://10.0.2.2:5000/api/signin",
        headers: {
          "Authorization": apiKey
        },
        body: jsonEncode({
  ...
  Future<User> registerUser(String username, String firstname, String lastname, String password, String email) async {
    // print("\n\n\n\nentered\n\n\n");
    final response = await client
        .post("http://http://192.168.19.66:5000/api/register",//"http://10.0.2.2:5000/api/register",
        //headers: {""},
        body: jsonEncode({
          "emailadress": email,
	        "username": username,
